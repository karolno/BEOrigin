...
sample=AHM0062
gender=male
reference=LP6008264-DNA_B06 # D2
tumour1=SLX-16213.DNAA005 # NE
tumour2=SLX-16213.DNAA006 # GC
tumour3=LP6008280-DNA_C06 # BE
...
#
#######################################################
# This patient did not require rerunning
#######################################################
...
echo "Running sample:" $sample >${DPout}/${sample}/stdout.log
echo "Running sample:" $sample >${DPout}/${sample}/stderr.log
Rscript ~/Dropbox/Postdoc/git/BEOrigin/Analysis/Battenberg/DPClust_final_GC.R -s $sample -d $DPout -g $gender >>${DPout}/${sample}/stdout.log 2>>${DPout}/${sample}/stderr.log 
...
echo "Running sample:" $sample >${DPout}/${sample}/stdout.log
echo "Running sample:" $sample >${DPout}/${sample}/stderr.log
Rscript ~/Dropbox/Postdoc/git/BEOrigin/Analysis/Battenberg/DPClust_final_NE.R -s $sample -d $DPout -g $gender >>${DPout}/${sample}/stdout.log 2>>${DPout}/${sample}/stderr.log 

#######################################################
# Check the solution of the above runs.
# The major peak of the BE sample should be aruond 1
# In this case it looks like the reads are not there and I will refit the data for BE. 
# This BE case did not have copy number changes to it might be the reason for fitting with high cellularity
#######################################################
# Only samples AHM0062 requires rerunning of BE
#######################################################
BEcell=0.72
# Run battenberg script
mkdir -p ${dirout}/${tumour3}_vs_${reference}_refit
# Run battenberg script
echo "Running sample:" $tumour3 >${dirout}/${tumour3}_vs_${reference}_refit/stdout.log
echo "Running sample:" $tumour3 >${dirout}/${tumour3}_vs_${reference}_refit/stderr.log
Rscript ~/Dropbox/Postdoc/git/BEOrigin/Analysis/Battenberg/battenberg_processing_rerun.R -r $reference -t $tumour3 -o ${dirout} -c $BEcell -p 2  >>${dirout}/${tumour3}_vs_${reference}_refit/stdout.log 2>>${dirout}/${tumour3}_vs_${reference}_refit/stderr.log
# Run DPClust again for the BE sample 
DPout="/mnt/data/Karol/BE_origin/Newest/DPClust_BE/"
...
Rscript ~/Dropbox/Postdoc/git/BEOrigin/Analysis/Battenberg/DPClust_final_BE_refit.R -s $sample -d $DPout -g $gender >>${DPout}/${sample}_refit/stdout.log 2>>${DPout}/${sample}_refit/stderr.log 
