...
directory.out<-paste0(directory.out, sample)
...
for(tmp in samples$Pool[samples$Patient == sample & samples$Tissue == "BE"]) {
  ...
  runGetDirichletProcessInfo(loci_file=paste0(directory.out,"/counts/","loci", ".txt"),
                             ...
                             cellularity_file=paste0("/home/karolno/data/Karol/BE_origin/Newest/battenberg/out/",tmp, "_vs_", D2.sample, "/", tmp, "_rho_and_psi.txt"),
                             subclone_file=paste0("/home/karolno/data/Karol/BE_origin/Newest/battenberg/out/",tmp, "_vs_", D2.sample, "/", tmp, "_subclones.txt"),
                             gender=gender,
                             SNP.phase.file="NA",
                             mut.phase.file="NA",
                             output_file=paste0(directory.out,"/dpclust3d/",tmp, "_dpclust.txt"))
  settings.file[[tmp]]<-c("sample" = sample, "subsample" = tmp, "datafile" = paste0(tmp, "_dpclust.txt"), "cellularity" = read.delim(paste0("/home/karolno/data/Karol/BE_origin/Newest/battenberg/out/",tmp, "_vs_", D2.sample, "/", tmp, "_rho_and_psi.txt"))["FRAC_GENOME", "rho"])
}
